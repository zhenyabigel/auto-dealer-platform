...
class SupplierFactory(factory.django.DjangoModelFactory):
    ...
    year_established = factory.LazyFunction(lambda: fake.random_int(min=1950, max=2020))
    discount_for_dealers = factory.LazyFunction(lambda: fake.random_int(min=0, max=30))
...
class PromotionFactory(factory.django.DjangoModelFactory):
    ...
    end_date = factory.LazyAttribute(lambda o: timezone.now() + timedelta(days=7))
    discount_percent = factory.LazyFunction(lambda: fake.random_int(min=5, max=30))
    dealer = factory.SubFactory(DealerFactory)

...
class DealerFactory(factory.django.DjangoModelFactory):
    ...
    location = factory.LazyFunction(
        lambda: fake.random_element(elements=list(dict(countries).keys()))
    )
    ...
    preferred_car_brands = factory.LazyFunction(lambda: ["Toyota", "BMW"])
    preferred_car_characteristics = factory.LazyFunction(lambda: {"year__gte": 2020})
